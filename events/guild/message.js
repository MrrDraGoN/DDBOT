...
module.exports = async (client, message) => {
  try {
    ...
    //get the current prefix from the config/config.json
     ...
     if (cmd.length === 0){
      if(matchedPrefix.includes(client.user.id))
        return message.channel.send(new Discord.MessageEmbed()
          ...
          .setTitle(`Hugh? Zostałem pingowany? Dam ci pomoc`)
          .setDescription(`Aby zobaczyć wszystkie komendy: \`${prefix}help\``)
        );
      return;
      }
